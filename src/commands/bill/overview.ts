...
export default class BillOverview extends Command {
  static aliases = ['bill:ov']
  static args = {
    cycle: Args.string({description: '账期 YYYY-MM (默认上个月)', required: false}),
  }
  static description = '展示指定账期的账单总览 (按产品汇总)'
  static examples = ['<%= config.bin %> <%= command.id %>', '<%= config.bin %> <%= command.id %> 2025-04']

  public async run(): Promise<void> {
    const {args} = await this.parse(BillOverview)
    ...
    await new BillManager().showOverview(cycle)
  }
}
